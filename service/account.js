...
var account = {
  ...
  identity: function(req, res, next){
    ...
    workflow.on('patchUser', function() {
      ...
      var options = { select: 'username email twitter.id github.id facebook.id google.id tumblr.id phone zip address city state occupation otherSpecify markets whereHeardUs' };
    ...
    statuses.forEach(function(status, i, arr){
      queries.push(function(done){
        ...
        case 'PropertySubmitted':
          req.app.db.models.Property.find({ "user.id" : req.params.id }).count({}, function(err, count){
            if(err){
              return done(err);
            }
            counts[status] = count;
          ...
          break;
        case 'New':
          req.app.db.models.Property.find({ "user.id" : req.params.id, "status" : 'new' }).count({}, function(err, count){
            if(err){
              return done(err);
            }
            counts[status] = count;
        ...
        case 'ActivelyWorking':
          req.app.db.models.Property.find({ "user.id" : req.params.id, "status": 'inProgress' }).count({}, function(err, count){
            if(err){
              return done(err);
            }
            counts[status] = count;
          ...
          break;
        case 'OfferAccepted':
          req.app.db.models.Property.find({ "user.id" : req.params.id, "status": 'offerAccepted' }).count({}, function(err, count){
            if(err){
              return done(err);
            }
            counts[status] = count;
          ...
          break;
        case 'OfferRejected':
          req.app.db.models.Property.find({ "user.id" : req.params.id, "status": 'offerRejected' }).count({}, function(err, count){
            if(err){
              return done(err);
            }
            counts[status] = count;
          ...
          break;
        case 'UCSeller':
          req.app.db.models.Property.find({ "user.id" : req.params.id, "status": 'ucSeller' }).count({}, function(err, count){
            if(err){
              return done(err);
            }
            counts[status] = count;
          ...
          break;
        case 'UCBuyer':
          req.app.db.models.Property.find({ "user.id" : req.params.id, "status": 'ucBuyer' }).count({}, function(err, count){
            if(err){
              return done(err);
            }
            counts[status] = count;
          ...
          break;
        case 'Closed':
          req.app.db.models.Property.find({ "user.id" : req.params.id, "status": 'closed' }).count({}, function(err, count){
            if(err){
              return done(err);
            }
            counts[status] = count;
          ...
          break;
        case 'DeadLeads':
          req.app.db.models.Property.find({ "user.id" : req.params.id, "status": 'deadLeads' }).count({}, function(err, count){
            if(err){
              return done(err);
            }
            counts[status] = count;
